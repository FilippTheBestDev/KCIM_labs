...
Phase3.prototype.canPlace = function(piece, isFirstHold = true) {
  const t = piece.type;
  const S = this.SZT;
  const L = this.LO;
  const I = this.IJ;
  ...
  if (t === "J") {
    if (isFirstHold && botMemory.holdPiece && this.canPlace(botMemory.holdPiece, false)
        && this.IJ.I_cnt === 1) return false;
    return true;
  }

  if (t === "I") {
    if (isFirstHold && botMemory.holdPiece && this.canPlace(botMemory.holdPiece, false)
        && this.IJ.J_cnt === 1) return false;
    return true;
  }
...
Phase3.prototype.execute = function(piece) {
  this.turn++;
  const t = piece.type;
  const S = this.SZT;
  const L = this.LO;
  ...
  if (t === 'I') {
    if (this.IJ_filled_times === 1 && this.IJ.I_cnt === 1) {
      ...
      if (findFirstCollisionY(grid, 4) >= 19)
        x = 4;
      else
        x = 5;
      x = 4 + (this.IJ.I_cnt % 2);
  ...
  // T rules - priority chain
    ...
    if (S.bottomT === 0) {
      ...
      x=7; y=findFirstCollisionY(grid, x)-1; rotation=0;
    } else {
  ...
  // S rules
    ...
    if (S.rightS < 2) {
      ...
      x=8; y=findFirstCollisionY(grid, 9)-3; rotation=1;
    } else if (S.rightS === 2 && S.leftTPlaced === 1 && S.leftS < 2) {
      S.leftS++;
      console.log("[EXECUTE][S] leftS++ ->", S.leftS);
      x=6; y=findFirstCollisionY(grid, 7)-3; rotation=1;
    } else {
  ...
  // Z rules
    ...
    if (S.bottomT === 0) {
      ...
      x=6; y=findFirstCollisionY(grid, x)-3; rotation=1;
    } else if (S.leftZ === 2 && S.rightTPlaced === 1 && S.rightZ < 2) {
      S.rightZ++;
      console.log("[EXECUTE][Z] rightZ++ ->", S.rightZ);
      x=8; y=findFirstCollisionY(grid, x)-3; rotation=1;
    } else {
  ...
  // LOJ updates:
    ...
    x = 0; y = findFirstCollisionY(grid, x)-2; rotation = 0;
  }
    ...
    x = 2; y = findFirstCollisionY(grid, x+1)-3; rotation = 3 - 2 * (this.LO.L_cnt % 2);
  }
